...
def process_audio_file(file_path, output_dir):
    ...
    num_samples = waveform.size(1)  # Number of samples in the waveform
    length_seconds = num_samples / sample_rate
    if (length_seconds <= 31):
        ...
        segment_save_path = os.path.join(output_dir, "segment_0.npy")
        extract_features_from_segment(waveform, sample_rate, segment_save_path)
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith('.mp3'):
                ...
                if gpu == 0:
                    if int(file_path.split("/")[-2]) >=0 and int(file_path.split("/")[-2]) < 100:
                        relative_path = os.path.relpath(file_path, input_dir)
                        output_file_dir = os.path.join(output_dir, os.path.splitext(relative_path)[0])
                        os.makedirs(output_file_dir, exist_ok=True)
                        pp = os.path.join(output_file_dir, "segment_0.npy")
                ...
                # ../dataset/jamendo/mp3/59/1227459.mp3
                # assert(False)
