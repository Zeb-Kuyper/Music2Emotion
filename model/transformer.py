...
class LatentTransformerClassifier(nn.Module):
    def __init__(self, input_size=72, num_heads=8, num_layers=6, d_model=512, dropout=0.5):
        ...
        self.pos_encoder = nn.Parameter(torch.zeros(1, 2136, d_model))  # assuming maximum sequence length is 500
    ...
    def forward(self, x):
        ...
        x_mood1 = self.fc_mood1(x)
        x_mood2 = self.fc_mood2(x_mood1)
        
        x_genre1 = self.fc_genre1(x)
        x_genre2 = self.fc_genre2(x_genre1)
        
        # Apply sigmoid to get the final logits
        logit_emo = torch.sigmoid(x_mood2)
        logit_genre = torch.sigmoid(x_genre2)
        
        return logit_emo, logit_genre



# class MusicEmotionTransformer(nn.Module):
#     def __init__(self, n_layers=6, num_heads=8, d_model=512, dim_feedforward=1024,
#                  dropout=0.1, max_sequence_midi=2048, max_sequence_chord=300):
#         super(MusicEmotionTransformer, self).__init__()
...
#         self.max_input_seq    = DAC_LATENTS_SIZE
...
#         self.positional_encoding = PositionalEncoding(self.d_model, self.dropout, self.max_input_seq)
