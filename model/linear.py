...
class FeedforwardModel(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.model = nn.Sequential(
            nn.Linear(input_size, 1024),  # Increased the first layer's units
            nn.BatchNorm1d(1024), 
            nn.ReLU(),
    ...
    # ]nn.Sequential(
    #         nn.Linear(8960, 1024),                # Increased the first layer's units
    #         nn.BatchNorm1d(1024),                 # Batch normalization after linear layer
    #         nn.ReLU(),
    #         nn.Dropout(0.3),                      # Adjusted dropout rate
    ...
    #         nn.Linear(128, self.adapt_classes(config.subset)),
    #     )