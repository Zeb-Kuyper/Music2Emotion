...
#     """Attention pooling to aggregate chord features dynamically."""
...
#     def forward(self, x):
#         weights = self.attention(x)  # Shape: (batch_size, seq_len, 1)
#         return (weights * x).sum(dim=1)  # Shape: (batch_size, input_dim)
...
class FeedforwardModelAttnCK(nn.Module):
    def __init__(self, input_size, output_size, nhead=8, num_layers=1, dropout_rate=0.1, 
        ...
        #self.d_model_transformer = chord_emb_dim

        # Embedding layers for chords and keys
        # self.chord_embedding = nn.Embedding(num_chords, chord_emb_dim)
        ...
        # self.key_embedding = nn.Embedding(num_key, key_emb_dim)

        # nn.init.xavier_uniform_(self.chord_embedding.weight)
        ...
        # self.positional_encoding = PositionalEncoding(chord_emb_dim)
        ...
        # self.input_proj = nn.Linear(input_size  + (chord_root_emb_dim + chord_attr_emb_dim) + key_emb_dim, self.d_model)
        # # Input projection for latent features
    ...
    def forward(self, model_input_dic ):
        ...
        # x_chord = model_input_dic["x_chord"]
        x_chord_root = model_input_dic["x_chord_root"]
        x_chord_attr = model_input_dic["x_chord_attr"]
        x_key = model_input_dic["x_key"]

        # key_embedding = self.key_embedding(x_key)
        # key_embedding = key_embedding.squeeze(1)

        key_embedding = x_key.float()

        #chord_embedding = self.chord_embedding(x_chord)  # Shape: (batch_size, seq_len, chord_root_emb_dim)
        ...
        # chord_combined_embedding = self.positional_encoding(chord_embedding)
        ...
        # chord_embedding_agg = chord_embedding_transformed[:,1:,:].mean(dim=1)  # Shape: (batch_size, chord_emb_dim)
