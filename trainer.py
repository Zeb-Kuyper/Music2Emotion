...
from model.linear_large import FeedforwardModelLarge
from model.linear_attn import FeedforwardModelAttn
from model.linear_attn_ck import FeedforwardModelAttnCK
from model.linear_ck import FeedforwardModelCK

from model.linear_mt import FeedforwardModelMT
from model.linear_mt_v2 import FeedforwardModelMTV2
from model.linear_mt_small import FeedforwardModelMTSmall
from model.linear_mt_small_v2 import FeedforwardModelMTSmallV2
from model.linear_mt_large import FeedforwardModelMTLarge
from model.linear_mt_attn import FeedforwardModelMTAttn
...
class MusicClassifier(pl.LightningModule):
    def __init__(self, cfg: DictConfig, output_file = None):
        ...
        mert_dim = 768 * layer_size # 768 1536 2304 ...
        ...
        if "mt" in self.classifier:
            ...
            classifiers = {
                "linear-mt": FeedforwardModelMT,
                "linear-mt-v2": FeedforwardModelMTV2,
                "linear-mt-small": FeedforwardModelMTSmall,
                "linear-mt-small-v2": FeedforwardModelMTSmallV2,
                "linear-mt-large": FeedforwardModelMTLarge,
                "linear-mt-attn": FeedforwardModelMTAttn,
            ...
            classifiers = {
                ...
                "linear-large": FeedforwardModelLarge,
                "linear-small": FeedforwardModelSmall,
                "linear-attn": FeedforwardModelAttn,
                "linear-attn-ck": FeedforwardModelAttnCK,
                "linear-ck": FeedforwardModelCK,
            }
        ...
        if self.kd:
            for i in range (self.num_datasets):
                if self.cfg.datasets[i] == "jamendo":
                    model_t_mood_path = self.cfg.checkpoint_jamendo
                elif self.cfg.datasets[i] == "emomusic":
                    model_t_va_path = self.cfg.checkpoint_emomusic
                elif self.cfg.datasets[i] == "pmemo":
                    model_t_va_path = self.cfg.checkpoint_pmemo
                elif self.cfg.datasets[i] == "deam":
                    model_t_va_path = self.cfg.checkpoint_deam

                teacher_model_VA = FeedforwardModelAttnCK(
                    input_size=self.input_size,
                    output_size_classification=56,
                    output_size_regression=2
                )
                teacher_model_mood = FeedforwardModelAttnCK(
                    input_size=self.input_size,
                    output_size_classification=56,
                    output_size_regression=2
                )

                self.teacher_model_VA = torch.load(model_t_va_path, map_location=self.device)
                self.teacher_model_mood = torch.load(model_t_mood_path, map_location=self.device)
                self.teacher_model_VA.eval()
                self.teacher_model_mood.eval()
    ...
    def handle_dataset(self, dataset_name, batch, losses, total_loss, stage):
        ...
        if "mt" in self.classifier:            
            if dataset_name == "jamendo":
                ...
                loss_classification, loss_regression = self.compute_mt_loss(model_input_dic, y_mood, y_va)
                ...
                if stage == "train":
                    ...
                    # losses['loss_mood'] = loss_classification
                    losses['loss_va'] = loss_regression
                    total_loss += (1- alpha) * loss_classification +  alpha * loss_regression
