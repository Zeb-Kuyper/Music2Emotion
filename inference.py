...
if __name__ == '__main__':
    ...
    path_temp_out = "./inference/temp_out"
    path_temp_out_mert = "./inference/temp_out/mert"
    path_output = "./inference/temp_out"
    path_input_audio = "./inference/input/test.mp3"

    if not os.path.exists(path_temp_out):
        os.makedirs(path_temp_out)
    
    if not os.path.exists(path_temp_out_mert):
        os.makedirs(path_temp_out_mert)
    
    parser = argparse.ArgumentParser()
    parser.add_argument('--audio_path', type=str, default='./inference/input/test.mp3')
    parser.add_argument('--model_path', type=str, default='./saved_models/J_all.ckpt')
    ...
    feature_extractor = FeatureExtractorMERT(model_name='m-a-p/MERT-v1-95M', device=device, sr=resample_rate)
    ...
    if is_split:        
        ...
        for i, segment in enumerate(segments):
            segment_save_path = os.path.join(path_temp_out_mert, f"segment_{i}.npy")
            feature_extractor.extract_features_from_segment(segment, sample_rate, segment_save_path)
    else:
        segment_save_path = os.path.join(path_temp_out_mert, f"segment_0.npy")
        feature_extractor.extract_features_from_segment(waveform, sample_rate, segment_save_path)

    embeddings = []
    layers_to_extract = [5,6]
    segment_embeddings = []
    for filename in sorted(os.listdir(path_temp_out_mert)):  # Sort files to ensure sequential order
        file_path = os.path.join(path_temp_out_mert, filename)
    ...
    final_embedding_mert.to(device)
    ...
    save_path = os.path.join(path_temp_out, os.path.split(audio_path)[-1].replace('.mp3', '').replace('.wav', '') + '.lab')
    ...
    mode_tensor = torch.tensor([encoded_mode], dtype=torch.long).to(device)
    ...
    # Convert to tensor
    chords_tensor = torch.tensor(encoded_chords, dtype=torch.long).to(device)
    chords_root_tensor = torch.tensor(encoded_chords_root, dtype=torch.long).to(device)
    chords_attr_tensor = torch.tensor(encoded_chords_attr, dtype=torch.long).to(device)
    
    music2emo_model = FeedforwardModelMTAttnCK(
        input_size= 768 * 2,
    ...
    model_keys = set(music2emo_model.state_dict().keys())
    ...
    music2emo_model.load_state_dict(filtered_state_dict)
    
    music2emo_model.to(device)
    music2emo_model.eval()
    ...
    # assert(False)
    # model_input_dic = {}
    # model_input_dic["x_mert"] = final_embedding_mert
    # model_input_dic["x_chord"] = chords_tensor
    # model_input_dic["x_chord_root"] = chords_root_tensor
    # model_input_dic["x_chord_attr"] = chords_attr_tensor
    # model_input_dic["x_key"] = mode_tensor

    model_input_dic = {k: v.to(device) for k, v in model_input_dic.items()}


    classification_output, regression_output = music2emo_model(model_input_dic)
    probs = torch.sigmoid(classification_output)

    logger.info("classificaion output")
